package net.chukkthedukk.heelermod.entity.custom;
...
public class HeelerEntity extends TameableEntity implements IAnimatable {
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        HeelerEntity baby = ModEntities.HEELER.create(world);
        HeelerVariant variant = Util.getRandom(HeelerVariant.values(), this.random );
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.heeler.walk", true));
            return PlayState.CONTINUE;
        }

        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.heeler.sit", true));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.heeler.idle", true));
        return PlayState.CONTINUE;
    }
    ...
    private static final TrackedData<Integer> DATA_ID_TYPE_VARIANT =
            DataTracker.registerData(HeelerEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    public HeelerVariant getVariant() {
        return HeelerVariant.byId(this.getTypeVariant() & 255);
    }
    ...
    private void setVariant(HeelerVariant variant) {
